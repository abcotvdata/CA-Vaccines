---
title: "COVID-19 HOSPITALIZATIONS BY VACCINATION STATUS"
subtitle: "7-DAY AVERAGE HOSPITALIZATIONS PER 100K PEOPLE"
output: 
  html_document:
...
hosp_filter <- ca_post_vax %>% 
  filter(date <= (most_recent-14))

ca_plot <- ggplot() +
  geom_path(data=hosp_filter,
            lineend = "butt",
            linejoin = "round",
            aes(x=date,
                y=vax_hosp_rate,
                group=1,
                text = paste(sep = "","For week ending: ", pretty_date ,"<br> 7-day average hospitalizations per 100K vaccinated people: ", round(vax_hosp_rate, digits=1)),
                color="Vaccinated",
                ), 
            size = 1.2) +
  geom_path(data=hosp_filter,
            ...
            aes(x=date, 
                y=unvax_hosp_rate,
                group=1,
                text = paste(sep = "", "For week ending: ", pretty_date ,"<br> 7-day average hospitalizations per 100K unvaccinated people: ", round(unvax_hosp_rate, digits=1)),
                color="Unvaccinated",
                ),
            size = 1.2) +
  geom_path(data=hosp_filter,
            ...
            aes(x=date, 
                y=boost_hosp_rate,
                group=1,
                text = paste(sep = "", "For week ending: ", pretty_date ,"<br> 7-day average hospitalizations per 100K boosted people: ", round(unvax_case_rate, digits=1)),
                color="Boosted",
                ),
            size = 1.2) +
  #labs(color = "Legend") +
...
### `r abs(more_likely_hosp)`x

</div>

<div class="panel-desc">

### MORE LIKELY TO BE HOSPITALIZED FROM COVID-19

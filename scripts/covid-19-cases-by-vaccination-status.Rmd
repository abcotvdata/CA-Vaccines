---
title: "COVID-19 CASES BY VACCINATION STATUS"
subtitle: "7-DAY AVERAGE CASES PER 100K PEOPLE"
output: 
  html_document:
...
cases_filter <- ca_post_vax %>% 
  filter(date <= (most_recent-7))

ca_plot <- ggplot() +
  geom_path(data=cases_filter,
            lineend = "butt",
            linejoin = "round",
            aes(x=date,
                y=vax_case_rate,
                group=1,
                text = paste(sep = "","For week ending: ", pretty_date ,"<br> 7-day average cases per 100K vaccinated people: ", round(vax_case_rate, digits=1)),
                color="Vaccinated",
                ), 
            size = 1.2) +
  geom_path(data=cases_filter,
            ...
            aes(x=date, 
                y=unvax_case_rate,
                group=1,
                text = paste(sep = "", "For week ending: ", pretty_date ,"<br> 7-day average cases per 100K unvaccinated people: ", round(unvax_case_rate, digits=1)),
                color="Unvaccinated",
                ),
            size = 1.2) +
  geom_path(data=cases_filter,
            ...
            aes(x=date, 
                y=boost_case_rate,
                group=1,
                text = paste(sep = "", "For week ending: ", pretty_date ,"<br> 7-day average cases per 100K boosted people: ", round(unvax_case_rate, digits=1)),
                color="Boosted",
                ),
            size = 1.2) +
  #labs(color = "Legend") +
